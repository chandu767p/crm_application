...
export default function Sidebar({ isOpen, onClose }) {
  ...
  const filteredNav = navItems.filter(item => userPermissions.includes(item.screen));
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 px-2.5 py-3 space-y-0.5 overflow-y-auto custom-scrollbar">
          {filteredNav.map(item => (
            <NavLink
              ...
              to={item.path}
              onClick={onClose}
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-semibold tracking-wider transition-all duration-150"
              style={({ isActive }) => ({
        ...
        <div className="px-3 py-3 border-t space-y-2" style={{ borderColor: 'var(--sidebar-border)' }}>
          ...
          {/* User */}
          <div className="flex items-center gap-3 px-3 py-2 rounded-xl cursor-pointer border" style={{ background: 'var(--bg-card)', borderColor: 'var(--sidebar-border)' }}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold truncate" style={{ color: 'var(--navbar-text)' }}>{user?.name}</p>
              <p className="text-xs truncate capitalize font-normal" style={{ color: 'var(--sidebar-text)' }}>{typeof userRole === 'string' ? userRole : userRole?.name}</p>
            </div>
            <button onClick={handleLogout} title="Logout" className="transition-colors" style={{ color: 'var(--sidebar-text)' }}>
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
